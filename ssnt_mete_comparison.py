...
def clean_data_genera(raw_data_site, min_genera = 5, min_sp = 10, max_removal = 0.05):
    """Further cleanup of data, removing individuals with undefined genus. 
    
    Inputs:
    raw_data_site - structured array generated by wk.import_raw_data(), with three columns 'site', 'sp', and 'size', for a single site
    ...
    Output:
    a structured array with four columns 'site', 'sp', 'genus', and 'size'
    ...
    for i, row in enumerate(raw_data_site):
        ...
        genus = sp_split[0]
        if len(sp_split) > 1 and genus[0].isupper() and genus[1].islower():
    ...
    if counter / len(raw_data_site) <= max_removal:
        ...
        if len(np.unique(out['sp'])) >= min_sp and len(np.unique(out['genus'])) >= min_genera: 
            return out
        else: return None
    else: return None
